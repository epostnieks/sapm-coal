...
const PSF_PARAMS = {pi_c:24.4,pi_p:61.0,w_c:122.0,kappa:0.68};
const PSF_DATA = [{pi:2.44,w:92.13},{pi:5.51,w:99.9},{pi:8.59,w:106.52},{pi:11.66,w:111.95},{pi:14.74,w:116.22},{pi:17.81,w:119.31},{pi:20.89,w:121.24},{pi:23.96,w:121.99},{pi:27.04,w:121.57},{pi:30.11,w:119.98},{pi:33.18,w:117.23},{pi:36.26,w:113.29},{pi:39.33,w:108.2},{pi:42.41,w:101.91},{pi:45.48,w:94.48},{pi:48.56,w:85.85},{pi:51.63,w:76.08},{pi:54.7,w:65.14},{pi:57.78,w:53.0},{pi:60.85,w:39.72},{pi:63.93,w:25.23},{pi:67.0,w:9.61},{pi:70.08,w:-7.23},{pi:73.15,w:-25.18},{pi:76.23,w:-44.37},{pi:79.3,w:-64.66}];

const MC_PARAMS = {n_draws:100000,mean:6.1,ci_lo:4.8,ci_hi:11.2,pct_hw:92.7,channels:[{name:"Primary welfare channel",dist:"LogNormal",lo:1.83,hi:4.27},{name:"Secondary externality",dist:"Triangular",lo:0.61,hi:2.44},{name:"Governance / institutional",dist:"Uniform",lo:0.3,hi:1.22}]};
const MC_DATA = [{bin:"0.3",count:147},{bin:"1.0",count:376},{bin:"1.6",count:858},{bin:"2.3",count:1752},{bin:"2.9",count:3203},{bin:"3.5",count:5245},{bin:"4.2",count:7690},{bin:"4.8",count:10097},{bin:"5.5",count:11872},{bin:"6.1",count:12499},{bin:"6.8",count:11783},{bin:"7.4",count:9948},{bin:"8.1",count:7520},{bin:"8.7",count:5091},{bin:"9.3",count:3086},{bin:"10.0",count:1675},{bin:"10.6",count:814},{bin:"11.3",count:354},{bin:"11.9",count:138},{bin:"12.6",count:48}];

const THRESHOLDS = [{domain:"Global Coal Combustion — regulatory trigger",year:2027,status:"Institutional response threshold approaching",confidence:"Medium",crossed:false},{domain:"Global Coal Combustion — welfare crossover",year:2031,status:"Projected welfare-cost crossover point",confidence:"High",crossed:false},{domain:"Global Coal Combustion — systemic failure",year:2036,status:"System-level failure if unaddressed",confidence:"Medium-Low",crossed:false},{domain:"Global Coal Combustion — irreversibility",year:2042,status:"Irreversible welfare destruction threshold",confidence:"Low",crossed:false}];

const AXIOMS = {type:"institutional",items:[{id:"I1",name:"Regulatory Capture",description:"Regulatory bodies governing Global Coal Combustion have been structurally captured by incumbent firms, preventing welfare-preserving intervention."},{id:"I2",name:"Information Asymmetry",description:"Welfare costs of Global Coal Combustion are distributed across populations that lack the information to price them, while benefits are concentrated."},{id:"I3",name:"Temporal Displacement",description:"Welfare destruction from Global Coal Combustion is temporally displaced — costs materialize years or decades after extraction, defeating feedback mechanisms."}]};

const METHODS_DATA = {
  welfare_function: "W(t) = W_0 - \u03A3_i \u03B2_W(i) \u00B7 \u03A0_i(t) + \u03B7 \u00B7 R(t), where R(t) represents remediation effort. For Global Coal Combustion, W_0 is calibrated to pre-extraction baseline welfare. The aggregate \u03B2_W = 6.1 implies $6.1 of welfare destruction per $1 of private payoff.",
  cooperative_baseline: "The cooperative extraction level \u03A0_C = $24.4B maximizes joint surplus W + \u03A0. Current extraction \u03A0_P = $61.0B exceeds this by $36.6B, representing the over-extraction gap. Shadow price \u03BC* = 0.1639 (16.4%) defines the efficient welfare price.",
  falsification: ["If \u03B2_W < 1.0 under any reasonable social cost methodology, the PST classification fails for Global Coal Combustion.","If welfare costs are fully internalized through existing market mechanisms (Coase conditions hold), the institutional failure claim is falsified.","If Monte Carlo robustness drops below 80% for \u03B2_W > 1.0, the point estimate is unreliable.","If cooperative baseline extraction \u03A0_C exceeds current \u03A0_P, over-extraction claim is reversed."],
  key_sources: ["Postnieks, E. (2026). System Asset Pricing Model: Global Coal Combustion. SAPM Working Paper.","Rennert, K. et al. (2022). Comprehensive evidence implies a higher social cost of CO\u2082. Nature 610.","Arrow, K. (1951). Social Choice and Individual Values. Impossibility theorem foundations.","Stiglitz, J. (2019). People, Power, and Profits. Institutional failure mechanisms.","Nordhaus, W. (2017). Revisiting the social cost of carbon. PNAS 114(7)."]
};
